    ...
    with open(log_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                if entry.get("success", True):  # 成功したものだけ除外
...
def get_topic():
    """未使用テーマを優先して選択。手動指定があればそれを使う"""
    override = os.environ.get("OVERRIDE_TOPIC", "").strip()
    if override:
        print(f"[INFO] 手動指定テーマ: {override}")
        return override

    used_topics = get_used_topics()
    print(f"[INFO] 使用済みテーマ数: {len(used_topics)}/{len(TOPICS)}")
    ...
    if unused:
        ...
        topic = unused[day % len(unused)]
        print(f"[INFO] 未使用テーマから選択: {topic}")
        return topic
    else:
        # 全テーマ使用済みならリセットして最初から
        print("[INFO] 全テーマ使用済み。リセットして最初から")
        day = datetime.date.today().timetuple().tm_yday
